      ...
      <li class="goods-options-list-item" v-for="(item,index) in optionsDatas" :key="index">
        <a class="goods-options-list-item-content" @click="onOptionsItemClick(item,index)">
          ...
          <span
            ...
            :class="[isShowSubContent && selectOptions.id === item.id ? 'goods-options-list-item-content-caret-open' : 'goods-options-list-item-content-caret-close']"
          ></span>
        </a>
      </li>
    </ul>
          ...
          >
            ...
            >
              <span
                ...
                :class="{'options-sub-content-list-item-content-name-active' :selectOptions.id === item.id}"
              >{{item.name}}</span>
              <img
                class="options-sub-content-list-item-content-select"
                src="../../assets/img/svg-icons/选中.svg"
  ...
  methods: {
    ...
    /*
        * 1.如果子选项处于展开状态，则关闭子选项视图
          ...
          并且当前item处于一个选中状态下的时候（item.id === selectOptions.id) 展示子选项视图
          3.设置选中项为用户点击的item
...
//   子选项内容展开动画，当v-if/v-show="true“  时候 调用
    ...
    // 子选项内容关闭的动画，当v-if/v-show="false" 时候调用
